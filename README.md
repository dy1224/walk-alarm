...
2단계 앱은 저장한 시각에 AlarmKit 시스템 알람을 매일 울린다. 알람 화면에는 "미션 시작" 버튼 하나만 있고,
...
- [ ] 알람 화면에 "미션 시작" 버튼 하나만 있고, 누르면 (잠금 해제 후) 앱이 열리며 "미션 시작으로 열렸어요"가 보인다
- [ ] 로그에 `테스트 알람 예약` → `alerting` → `미션 시작 버튼으로 앱 열림`이 남는다
