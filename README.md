...
| 2 | AlarmKit 알람 1개 + "미션 시작" 버튼으로 앱 열기 | 🔄 코드 완료 · 실기기 확인 대기 |
| 3 | 카메라 Bv 측정 + 실시간 그래프 + CSV 내보내기 | ⬜ |
| 4 | 상태 머신 + 앱 내 울림/일시정지 + 연쇄 알람 + "도착했어요" | ⬜ |

2단계 앱은 저장한 시각에 AlarmKit 시스템 알람을 매일 울린다. 알람 화면에는 "끄기"와 "미션 시작" 버튼이 있고,
"미션 시작"을 누르면 앱이 열린다. 디버그 화면의 "1분 뒤 알람 테스트"로 잠금·무음 상태에서 바로 확인할 수 있다.
목표는 **무료 Apple ID 서명에서 AlarmKit이 동작하는지** 확인하는 것. 동작하지 않으면 멈추고 대안을 정한다.
...
└── WalkAlarm/
    ...
    │   └── AlarmIntents.swift      알람 화면 버튼 ("미션 시작" = 앱 열기, "끄기")
    ├── Views/
...
- [ ] 알람을 켜면 AlarmKit 권한을 묻고, 허용하면 알람 화면에 "N시간 M분 뒤"가 보인다
- [ ] 디버그 화면 "알람 (AlarmKit)"에 권한 "허용됨", 매일 알람 1개(`scheduled`)가 보인다
- [ ] "1분 뒤 알람 테스트" → 화면 잠금 + 무음 스위치 켬 → 1분 뒤 시스템 알람이 소리와 함께 울린다
- [ ] 알람 화면에 "미션 시작" 버튼이 있고, 누르면 (잠금 해제 후) 앱이 열리며 "미션 시작으로 열렸어요"가 보인다
- [ ] 한 번 더 테스트해서 "끄기"로 끌 수 있다
- [ ] 로그에 `테스트 알람 예약` → `alerting` → `미션 시작 버튼으로 앱 열림` 또는 `시스템 알람 끄기 버튼`이 남는다
